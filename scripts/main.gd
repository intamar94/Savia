...
var blender_asset_loader: BlenderAssetLoader = null
var ecological_elements: EcologicalElements = null
...
func _create_menu_habitat() -> void:
	# Usar elementos procedurales en lugar de assets externos
	if not ecological_elements:
		ecological_elements = EcologicalElements.new()
	
	# Crear árboles alrededor del menú con elementos procedurales
	var tree_positions := [
		Vector3(6.0, -0.02, -4.0),
		Vector3(8.2, -0.02, -4.8),
		Vector3(10.2, -0.02, -6.0)
	]
	
	for i in range(tree_positions.size()):
		var pos = tree_positions[i]
		var scale_factor = 0.6 + (i * 0.1)
		var tree = ecological_elements.create_stylized_tree(pos, scale_factor)
		tree.rotation.y = float(i) * 1.2
		world_root.add_child(tree)
	
	# Agregar pequeño árbol ancla (cierre visual)
	var anchor_tree = ecological_elements.create_stylized_tree(Vector3(8.0, -0.02, -8.8), 1.10)
	anchor_tree.rotation.y = -0.55
	world_root.add_child(anchor_tree)
	
	# Agregar sistemas de raíces bajo los árboles
	for i in range(2):
		var root_system = ecological_elements.create_root_system(
			Vector3(6.5 + i * 3.0, -2.0, -5.0),
			0.8
		)
		world_root.add_child(root_system)
	
	# Agregar microorganismos en el suelo
	for i in range(3):
		var microbes = ecological_elements.create_microorganism_cluster(
			Vector3(6.0 + i * 2.0, -1.5, -4.5),
			8
		)
		world_root.add_child(microbes)
	
	# Agregar insectos voladores decorativos
	for i in range(4):
		var angle = (i / 4.0) * TAU
		var x = 7.5 + cos(angle) * 2.0
		var z = -5.0 + sin(angle) * 2.0
		var insect = ecological_elements.create_insect(Vector3(x, 0.5, z), 0.5)
		world_root.add_child(insect)
	
	# Agregar parche de suelo visible
	var soil_patch = ecological_elements.create_soil_patch(Vector3(7.5, -1.9, -5.0), Vector3(6.0, 0.5, 6.0))
	world_root.add_child(soil_patch)
	
	# Agregar hierba decorativa
	var grass = ecological_elements.create_grass_patch(Vector3(7.5, -1.75, -5.0), 6.0, 6.0)
	world_root.add_child(grass)
	
	# Pequeños puntos luminosos decorativos (bioluminiscencia)
	for i in range(6):
	...
	# EXTENSIVE VISIBLE ROOT NETWORK in menu area
	var extensive_roots := [
		# Major roots spreading down
		[Vector3(7.0, -0.02, -3.5), Vector3(7.5, -0.48, -4.2)],
		[Vector3(8.0, -0.02, -3.0), Vector3(8.2, -0.50, -4.5)],
		[Vector3(9.5, -0.02, -4.0), Vector3(9.8, -0.52, -5.0)],
		# Lateral root branches
		[Vector3(7.5, -0.35, -4.2), Vector3(6.2, -0.52, -4.8)],
		[Vector3(8.2, -0.40, -4.5), Vector3(9.5, -0.55, -5.2)],
		[Vector3(9.8, -0.48, -5.0), Vector3(11.2, -0.58, -5.8)],
		# Deep root extensions
		[Vector3(7.0, -0.48, -4.2), Vector3(6.8, -0.85, -4.5)],
		[Vector3(8.5, -0.50, -4.8), Vector3(8.8, -0.88, -5.3)],
		[Vector3(10.0, -0.52, -5.5), Vector3(10.5, -0.90, -6.0)]
	]
	for path in extensive_roots:
		_add_profile_root(world_root, path[0], path[1])

	# Bioluminescent root junctions - nodes where roots branch
	var root_junctions := [
		Vector3(7.5, -0.35, -4.2),
		Vector3(8.2, -0.40, -4.5),
		Vector3(9.8, -0.48, -5.0),
		Vector3(7.0, -0.48, -4.2),
		Vector3(8.5, -0.50, -4.8)
	]
	for junction_pos in root_junctions:
		...
		junction.material_override = _glow_mat(Color("#c7d88a"), Color("#e1ff99"), 1.2, 0.85)
		world_root.add_child(junction)

	# Insects actively exploring the menu zone
	var menu_insects := [
		Vector3(6.5, 0.8, -3.5),
		Vector3(8.0, 1.2, -4.2),
		Vector3(9.5, 0.9, -5.0),
	...
	for i in range(menu_insects.size()):
		var insect := Node3D.new()
		insect.position = menu_insects[i]
		insect.scale = Vector3.ONE * 1.2
		world_root.add_child(insect)
		...
		body.material_override = _mat(Color("#3a2f24"), 0.65)
		insect.add_child(body)

		for side in [-1.0, 1.0]:
			var wing := MeshInstance3D.new()
			var wing_mesh := QuadMesh.new()
			wing_mesh.size = Vector2(0.18, 0.12)
			wing.mesh = wing_mesh
			wing.position = Vector3(0, 0.015, side * 0.08)
			wing.rotation_degrees.y = 20.0 * side
			wing.material_override = _glow_mat(
				Color("#a8d9b8") if i % 2 == 0 else Color("#e8b85a"),
				Color("#c4ffe5") if i % 2 == 0 else Color("#ffd68a"),
				1.1, 0.80
			)
			insect.add_child(wing)

	# Microorganism clouds around roots in menu
	for i in range(18):
		var micro := MeshInstance3D.new()
		var mm := SphereMesh.new()
		mm.radius = 0.012 + float(i % 3) * 0.006
		...
		micro.material_override = _glow_mat(Color("#7dcc9f"), Color("#9fffb5"), 0.85, 0.75)
		world_root.add_child(micro)

	# Floating bio-particles flowing up from roots to menu
	for i in range(22):
		var particle := MeshInstance3D.new()
		var pm := SphereMesh.new()
		pm.radius = 0.008 + float(i % 3) * 0.004
		...
		particle.material_override = _glow_mat(Color("#a8ef85"), Color("#c8ff9e"), 1.3, 0.70)
		world_root.add_child(particle)
		light_particles.append(particle)

	# Luminous root detail: small glowing hairs/filaments
	for i in range(14):
		...
		fm.height = 0.18
		filament.mesh = fm
		filament.position = Vector3(
			7.2 + float(i % 4) * 0.65,
			-0.35 - float(i / 4) * 0.25,
			-4.5 + sin(i * 0.8) * 0.3
		)
		filament.rotation_degrees.z = float(i) * 22.5
		filament.material_override = _glow_mat(Color("#a8d88a"), Color("#d1f59c"), 0.9, 0.65)
		world_root.add_child(filament)
		root_nodes.append(filament)

	# Root zone lighting
	var menu_root_light := OmniLight3D.new()
	menu_root_light.position = Vector3(8.5, -0.40, -4.8)
	menu_root_light.light_color = Color("#9eff8d")
	menu_root_light.light_energy = 0.85
	menu_root_light.omni_range = 4.2
	world_root.add_child(menu_root_light)

	var menu_insect_light := OmniLight3D.new()
	menu_insect_light.position = Vector3(8.0, 1.0, -4.2)
	menu_insect_light.light_color = Color("#d4b88a")
	menu_insect_light.light_energy = 0.45
	menu_insect_light.omni_range = 3.0
	world_root.add_child(menu_insect_light)
...
func _create_living_soil_scene() -> void:
	...
	# Expanded microorganism network showing rich biological activity
	for i in range(28):
	...
	# ================================================================
	# CARGAR ASSETS DE BLENDER
	# ================================================================
	print("\n============================================================")
	print("🌱 CARGANDO ASSETS DE BLENDER...")
	print("============================================================")
	
	_load_blender_assets(world_root)
...
# Cargar assets .glb generados desde Blender
func _load_blender_assets(parent: Node3D) -> void:
	var project_root = ProjectSettings.globalize_path("res://")
	var assets_dir = project_root + "assets/blender_exports/"
	
	var files_to_load = [
		{"name": "roots", "file": "roots.glb", "pos": Vector3(0, -2.0, -5.8), "scale": 0.8},
		{"name": "microorganisms", "file": "microorganisms.glb", "pos": Vector3(0, -1.5, -5.8), "scale": 0.5},
		{"name": "insects", "file": "insects.glb", "pos": Vector3(0, 0.5, -5.8), "scale": 0.6}
	]
	
	for asset_data in files_to_load:
		var filepath = assets_dir + asset_data["file"]
		
		if not FileAccess.file_exists(filepath):
			print("⚠ Archivo no encontrado: ", filepath)
			continue
		
		print("📦 Cargando: ", asset_data["name"])
		...
		if err != OK:
			print("✗ Error al cargar: ", err)
			continue
		
		var scene := document.generate_scene(state)
		if scene:
			scene.position = asset_data["pos"]
			scene.scale = Vector3.ONE * asset_data["scale"]
			parent.add_child(scene)
			print("✓ Agregado: ", asset_data["name"])
		else:
			print("✗ No se pudo generar escena para: ", asset_data["name"])
